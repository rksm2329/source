...
struct SegTree {
  Node dat[MAXN << 2], E = {0, INF};
  Tag tag[MAXN << 2], I = {0};
  Node comb(const Node &dat1, const Node &dat2) {
    return {dat1.sum + dat2.sum, min(dat1.mi, dat2.mi), dat1.len + dat2.len};
  }
  Tag F(const Tag &tag1, const Tag &tag2) {
    return {tag1.val + tag2.val};
  }
  Node f(const Node &dat, const Tag &tag) {
    return {dat.sum + dat.len * tag.val, dat.mi + tag.val, dat.len};
  }
  void build(int root, int l, int r) {
    if (l == r) {
      cin >> dat[root].sum;
      dat[root].mi = dat[root].sum, dat[root].len = 1;
      return;
    }
    int mid = l + r >> 1;
    build(root << 1, l, mid);
